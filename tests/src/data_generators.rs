...
pub fn generate_concentric_circles(center: Coordinate<f64>, r_min: f64, r_max: f64, num_polys: usize, num_points: usize) -> MultiPolygon<f64> {
    assert!(r_max > r_min);
    assert!(r_min > 0.0);
    assert!(num_polys >= 1);
    assert!(num_points >= 3);
    ...
    let radii: Vec<_> = (0 .. num_radii).map(
        |i| r_min + (i as f64) * (r_max - r_min) / ((num_radii - 1) as f64)
    ).collect();
